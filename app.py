...
def forums_type(type):
    ...
    if type == 'music':
        if request.method == 'POST': 
            ...
            return render_template('forums-music.html',type=type, forums = forums, page_title='Music Forum')
    elif type == 'explore':
        if request.method == 'POST':
            title = request.form.get('title')
            user_id = session.get('user_id')
            if title:
                ...
                flash("Forum title required!")
        forums = music.load_forums(conn, type)
        return render_template('forums-explore.html', type=type, forums=forums, page_title='Explore Forum')
    elif type == 'beef':
        if request.method == 'POST':
            # want to select from the forums
            # or make a new forum
            ...
            if title:
                ...
                flash("Forum title required!")
        forums = music.load_forums(conn, type)
        return render_template('forum-beef.html',type=type, forums=forums, page_title='Beef Forum')
